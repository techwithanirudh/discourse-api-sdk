...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'put',
  httpPath: '/u/{username}/preferences/avatar/pick.json',
  operationId: 'updateAvatar',
};

export const tool: Tool = {
  name: 'update_preferences_u_avatar',
  description:
  ...
  inputSchema: {
    ...
    properties: {
      ...
      type: {
        type: 'string',
        enum: ['uploaded', 'custom', 'gravatar', 'system'],
      },
      upload_id: {
        type: 'integer',
      },
...
export const handler = async (client: DiscourseSDK, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(
    await maybeFilter(args, await client.u.preferences.avatar.update(username, body)),
  );
};
